...
class CalendarManager:
    ...
    def get_today_bounds(self):
        ...
        start = datetime.combine(now.date(), time.min).replace(tzinfo=timezone.utc).isoformat()
        end = datetime.combine(now.date(), time.max).replace(tzinfo=timezone.utc).isoformat()
        return start, end
    ...
    def manage_calendar(self, action: str, details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if action == "list":
            return {"status": "success", "data": self.list_today_events()}
    ...
    def get_today_summary(self, tasks_manager=None, sheets_manager=None, spreadsheet_id: str = None, log_range: str = "A1:Z100"):
